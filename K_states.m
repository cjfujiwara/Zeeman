...
f_s12=figure(10001);
f_s12.Color='w';
f_s12.Position=[50 50 400 300];
clf
inds = [1 2 17 18]; % Track a,b,q,r states
% inds = [2]; % Track a,b,q,r states
legStr={};
...
for mm=1:length(inds)
    ...
    legend(legStr,'interpreter','latex','location','east','fontsize',8);
end
set(gca,'box','on','linewidth',1,'fontsize',10,'fontname','times');
xlabel('field (G)');
ylabel('amplitude');
xlim([0 230]);
title('$^{40}\mathrm{K}~4 S_{1/2}$','interpreter','latex');
ylim([0 1]);
%%

f_p32=figure(10002);
f_p32.Color='w';
f_p32.Position=[50 350 400 300];
clf
inds = [1 2  36]; % Track a,b,q,r states
% inds = [2]; % Track a,b,q,r states
numstates=[1 2  4];
legStr={};
co=get(gca,'colororder');
...
ylabel('amplitude');
xlim([0 230]);
title('$^{40}\mathrm{K}~4 P_{3/2}$','interpreter','latex');
ylim([0 1]);
